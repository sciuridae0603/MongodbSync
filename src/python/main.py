...
logging.basicConfig(
    filename=g_log_file,
    format="[%(levelname)s] %(thread)d %(asctime)s %(module)s:%(funcName)s:%(lineno)d %(message)s",
    level=logging.INFO if not is_debug else logging.DEBUG,
)
logging.info(
    "config file: %s, log file: %s, is_debug: %s" % (g_conf_file, g_log_file, is_debug)
)
...
class MongodSynchronizer(object):
    def __init__(self, conf):
        ...
        for src_addr in src_addrs:
            src_addr = list(src_addr)
            if len(conf.mongo_src[0].split("@")) >= 2:
                src_addr[0] =conf.mongo_src[0].split("@")[0] + "@" + src_addr[0]

            if len(conf.mongo_src[0].split("?")) >= 2:
                # add authMechanism
                src_addr[0] = src_addr[0] + "?" + conf.mongo_src[0].split("?")[1]

            self._src_mongo_conns.append(MongoConnInfo(src_addr[0], src_addr[1]))

        src_mc.close()

        # dest mongodb conns
        self._dest_mongo_conn = MongoConnInfo(conf.mongo_dest[0])
    ...
    def _getReplInfo(self, client):
        ...
        if status["ok"] == 1:
            ...
            logging.info("status not ok: %s" % (status))
    ...
    def startSync(self):
        ...
        for src_conn in self._src_mongo_conns:
            t = replica_syn.ReplicaSynchronizer(
                src_conn, self._dest_mongo_conn, self._conf
            )
            t.start()
            threads.append(t)
